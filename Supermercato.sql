...
CREATE TABLE utente_backup (
nome VARCHAR(20) NOT NULL,
cognome VARCHAR(20) NOT NULL,
cod_fisc CHAR(16) NOT NULL PRIMARY KEY,
...
INSERT INTO utente_backup (nome, cognome, cod_fisc, email, telefono)
SELECT nome, cognome, cod_fisc, email, telefono FROM utente;


SELECT nome, cognome email, cod_fisc from utente;




SELECT s.nome, s.cognome, s.email, o.tipo_prodotto, o.prezzo
FROM utente s
INNER JOIN ordini o ON s.cod_fisc = o.cod_fisc;



SELECT s.nome, s.cognome, s.email, 
       COALESCE(o.tipo_prodotto, 'Nessun ordine') AS prodotto, 
       COALESCE(o.prezzo, 0) AS prezzo
FROM utente s
LEFT JOIN ordini o ON s.cod_fisc = o.cod_fisc;




 Rimuoviamo il vincolo di chiave esterna esistente dalla tabella `ordini`
ALTER TABLE ordini DROP FOREIGN KEY fk_ordini_utente;

Modifichiamo la colonna `cod_fisc` nella tabella `utente` da CHAR(16) a CHAR(36)
ALTER TABLE utente MODIFY cod_fisc CHAR(36) NOT NULL;

Modifichiamo la colonna `cod_fisc` nella tabella `ordini` da CHAR(16) a CHAR(36)
ALTER TABLE ordini MODIFY cod_fisc CHAR(36) NOT NULL;

 Ricreiamo il vincolo di chiave esterna per ristabilire il collegamento tra `ordini` e `utente`
ALTER TABLE ordini
ADD CONSTRAINT cod_fisc FOREIGN KEY (cod_fisc)
REFERENCES utente(cod_fisc);
...
FROM utente
WHERE cod_fisc NOT IN (SELECT cod_fisc FROM ordini);




SELECT nome, cognome, email, (SELECT SUM(prezzo) FROM ordini WHERE ordini.cod_fisc = utente.cod_fisc) AS totale_speso
FROM utente
...
SELECT nome, cognome, email,
(SELECT prezzo FROM ordini o WHERE o.cod_fisc = utente.cod_fisc LIMIT 1) AS primo_prezzo,
(SELECT tipo_prodotto FROM ordini o WHERE o.cod_fisc = utente.cod_fisc LIMIT 1) AS primo_tipo_prodotto
FROM utente;
...
// utenti chi non ha ordinato 
SELECT u.nome, u.cognome, u.email
FROM utente u
LEFT JOIN ordini o ON u.cod_fisc = o.cod_fisc
WHERE o.cod_fisc IS NULL;

...
<body>
    <div class="container">
        ...
        <label for="d1">Data di nascita:</label>
        <input type="date" id="d1">

        <label for="mySelect">Sesso:</label>
        <select id="mySelect">
        ...
        <label for="catastale">Codice catastale del comune:</label>
        <input type="text" id="catastale">

        <button onclick="invia()">Calcola Codice Fiscale</button>
        <button onclick="scarica()">Download</button>
    </div>

    <!-- Output opzionale -->
    <p id="output"></p>

    <!-- Il tuo script JS va qui sotto -->
    <script>
         var link = "";
        function Codice(){
var vocali=0;
...
//txt.innerHTML += codice_fiscale.charAt(i); 
    //console.log(codice_fiscale.charAt(i));

}

// Recupera i dati precedenti da localStorage (se esistono)
var cod = JSON.parse(localStorage.getItem('cod') || '[]'); // Se non ci sono dati, inizia con un array vuoto

// Costruzione oggetto JSON con i dati
var datiUtente = {
    cognome: cognome,
    nome: nome,
    data_nascita: data,
    sesso: sesso,
    codice_catastale: comune,
    codice_fiscale: codice_fiscale
};

// Aggiungi il nuovo record all'array
cod.push(datiUtente);
...
var jsonData = JSON.stringify(cod); // Serializza l'intero array
